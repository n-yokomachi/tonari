...
def _build_news_prompt(now_str: str, urgent_tasks: list[dict] | None = None) -> str:
    ...
    return (
        ...
        "## 最重要ルール\n"
        "1. このタスクでは TavilySearch___TavilySearchPost ツールの使用が【絶対必須】です。\n"
        "   あなたはニュースを一切知りません。記憶にあるニュース情報はすべて古く無効です。\n"
        "   必ず TavilySearch___TavilySearchPost ツールを実行し、その検索結果だけからニュースを構成してください。\n"
        "   ツールを実行せずにニュースを出力した場合、その内容は100%古い情報であり、オーナーに誤情報を届けることになります。\n"
        "2. 長期記憶（LTM）・会話履歴・過去のセッションに含まれるニュース情報は【すべて無視】してください。\n"
        "   記憶の中に「〜のニュースまとめ」「〜のニュース配信」等の情報があっても、それは過去の古い情報です。\n"
        "   それらを参考にしたり、引用したり、再利用することは絶対に禁止です。\n"
        "   ニュースの情報源は TavilySearch の検索結果【のみ】です。それ以外の情報源からニュースを生成してはいけません。\n"
