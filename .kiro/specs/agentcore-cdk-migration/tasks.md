...
- [ ] 1. (P) Agent コンテナ用 Dockerfile を作成する
...
- [ ] 2. CognitoConstruct を実装する
- [ ] 2.1 (P) User Pool、Resource Server、App Client、Domain を作成するコンストラクトを実装する
...
- [ ] 2.2 App Client の Client Secret を SSM Parameter Store に格納する仕組みを追加する
...
- [ ] 3. (P) WorkloadConstruct を実装する（既存リソースの移設）
...
- [ ] 4. AgentCoreConstruct を実装する
- [ ] 4.1 (P) AgentCore Memory リソースを CDK で作成する
...
- [ ] 4.2 (P) ECR リポジトリ、CodeBuild プロジェクト、ビルドトリガーを作成する
  - ECR リポジトリを作成する
...
- [ ] 4.3 MCP Gateway と Lambda ターゲットを作成する
...
- [ ] 4.4 AgentCore Runtime をコンテナデプロイモードで作成する
...
- [ ] 4.5 Observability（CloudWatch Logs + X-Ray）を構成する
...
- [ ] 5. TonariStack をオーケストレーション層にリファクタする
- [ ] 5.1 TonariStack を各コンストラクトの組み立てと接続のみに書き換える
...
- [ ] 5.2 全新規リソースの CfnOutput を追加する
  - Runtime ARN、Memory ID、Gateway URL、Cognito User Pool ID、Cognito Client ID、Token Endpoint、Scope を出力する
  - _Requirements: 7.1_

- [ ] 6. CDK エントリポイントと設定ファイルを更新する
- [ ] 6.1 (P) `infra/bin/infra.ts` から不要な context 参照を削除する
...
- [ ] 6.2 (P) `infra/cdk.json` の context を整理する
