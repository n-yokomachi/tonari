...
- [x] 1. Settings Store拡張 — TTS設定項目の追加
- [x] 1.1 TTSエンジン切り替え・AivisSpeech接続・話者設定をSettings Storeに追加する
...
- [x] 2. AivisSpeech APIクライアント実装
- [x] 2.1 (P) AivisSpeech Engineの2ステップAPI呼び出し関数を作成する
...
- [x] 3. speakCharacter TTSエンジン分岐の統合
- [x] 3.1 音声合成リクエスト時にTTSエンジン設定に基づいてAivisSpeechまたはPollyのパスに分岐する
...
- [x] 3.2 AivisSpeech API通信失敗時にテキストベースリップシンクへフォールバックする
  - AivisSpeechのfetch失敗をcatchし、既存の `speakWithTextLipSync` を呼び出す
  - console.errorでエラー内容をログ出力
  - _Requirements: 2.4_

- [x] 4. (P) 設定UI拡張 — TTS設定画面の追加
- [x] 4.1 音声出力設定セクションにTTSエンジン切り替えUIを追加する
...
- [x] 5. (P) AivisSpeech Engineセットアップドキュメント作成
- [x] 5.1 Docker起動手順・音声モデル導入・トラブルシューティングを記載したセットアップガイドを作成する
